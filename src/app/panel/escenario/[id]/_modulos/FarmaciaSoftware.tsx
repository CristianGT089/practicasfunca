...
export default function FarmaciaSoftware({ intentoId }: { intentoId: string }) {
  ...
  async function solicitarCedula() {
    const res = await fetch(`/api/modulos/farmacia/intentos/${intentoId}/solicitar-cedula`, { method: "POST" });
    const data = await res.json();
    if (typeof data.vidas === "number") setVidas(data.vidas);
    if (data.progreso) setProgreso(data.progreso);
    if (typeof data.precision === "number") setPrecision(data.precision);
  ...
  async function buscarFichaPaciente() {
    ...
    const res = await fetch(`/api/modulos/farmacia/intentos/${intentoId}/buscar-paciente`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function buscarRecetaOnline() {
    const res = await fetch(`/api/modulos/farmacia/intentos/${intentoId}/receta-online`, { method: "POST" });
    const data = await res.json();
    if (data.error) {
      setAvisoCedula(data.error);
      setTimeout(() => setAvisoCedula(null), 6000);
